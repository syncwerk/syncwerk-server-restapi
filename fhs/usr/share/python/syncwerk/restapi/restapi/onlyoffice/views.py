...
from restapi.onlyoffice.settings import VERIFY_ONLYOFFICE_CERTIFICATE
from restapi.utils import gen_inner_file_upload_url
...
def onlyoffice_editor_callback(request):
    ...
    if status in (2, 6):
        ...
        update_token = syncwerk_api.get_fileserver_access_token(repo_id,
        ...
        # get file content
        files = {
            'file': requests.get(url, verify=VERIFY_ONLYOFFICE_CERTIFICATE).content,
        ...
        requests.post(update_url, files=files, verify=VERIFY_ONLYOFFICE_CERTIFICATE)
