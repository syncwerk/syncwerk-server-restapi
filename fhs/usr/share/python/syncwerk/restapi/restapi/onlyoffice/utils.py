...
from restapi.utils import get_file_type_and_ext, gen_file_get_url, \
        get_site_scheme_and_netloc

from restapi.settings import ENABLE_WATERMARK
...
def get_onlyoffice_dict(username, repo_id, file_path,
    ...
    if repo.is_virtual:
        ...
        if not file_id:
            file_id = syncwerk_api.get_file_id_by_path(origin_repo_id,
                    origin_file_path)
    else:
        origin_repo_id = repo_id
        origin_file_path = file_path
        if not file_id:
            file_id = syncwerk_api.get_file_id_by_path(repo_id,
                    file_path)

    dl_token = syncwerk_api.get_fileserver_access_token(repo_id,
    ...
    onlyoffice_editor_callback_url = reverse('api3_onlyoffice_editor_callback')
