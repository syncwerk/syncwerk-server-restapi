...
from restapi.base.accounts import User, ANONYMOUS_EMAIL
...
from restapi.wopi.utils import get_file_info_by_token
...
def access_token_check(func):
    ...
    def _decorated(view, request, file_id, *args, **kwargs):
        ...
        if not obj_id:
            # if not cache file obj_id, then get it from syncwerk_api
            obj_id = syncwerk_api.get_file_id_by_path(repo_id, file_path)
...
class WOPIFilesView(APIView):
    ...
    def get(self, request, file_id, format=None):
        ...
        try:
            file_size = syncwerk_api.get_file_size(repo.store_id,
                    repo.version, obj_id)
        ...
        try:
            if is_pro_version():
                result['OwnerId'] = syncwerk_api.get_repo_owner(repo_id) or \
                        syncwerk_api.get_org_repo_owner(repo_id)
            else:
                result['OwnerId'] = syncwerk_api.get_repo_owner(repo_id)
        ...
        # new file creation feature is not implemented on wopi host(restapi)
...
class WOPIFilesContentsView(APIView):
    ...
    def get(self, request, file_id, format=None):
        ...
        try:
            fileserver_token = syncwerk_api.get_fileserver_access_token(repo_id,
