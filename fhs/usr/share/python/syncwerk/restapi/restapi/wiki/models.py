...
class WikiManager(models.Manager):
    def add(self, wiki_name, username, permission='private', repo_id=None,
        ...
        if repo_id is None:     # create new repo to store the wiki pages
            if org_id > 0:
                repo_id = syncwerk_api.create_org_repo(wiki_name, 'Wiki', username,
                                                      passwd=None, org_id=org_id)
            else:
                repo_id = syncwerk_api.create_repo(wiki_name, 'Wiki', username,
                                                  passwd=None)
...
class Wiki(models.Model):
    ...
    def updated_at(self):
        ...
        repo = syncwerk_api.get_repo(self.repo_id)
        if not repo:
            return ''
