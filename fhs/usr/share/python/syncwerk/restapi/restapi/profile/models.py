...
from restapi.profile.settings import EMAIL_ID_CACHE_PREFIX, EMAIL_ID_CACHE_TIMEOUT
from restapi.tenants.models import Tenant
from registration.signals import user_registered
from restapi.signals import tenant_deleted
...
class ProfileManager(models.Manager):
    def add_or_update(self, username, nickname=None, intro=None, lang_code=None,
                      login_id=None, contact_email=None, tenant=None):
...
class Profile(models.Model):
    ...
    tenant = models.CharField(max_length=225, db_index=True, null=True, blank=True, default='')
...
def remove_user_for_inst_deleted(sender, **kwargs):
    ...
    Profile.objects.filter(tenant=inst_name).update(tenant="")
