...
from restapi.api2.utils import api_error, send_perm_audit_signal
    ...
    def post(self, request):
        ...
        for repo_id in repo_id_list:
            ...
            if is_org_context(request):
                ...
                if not syncwerk_api.is_repo_owner(username, repo_id):
                    result['failed'].append({
                        'repo_id': repo_id,
                        'error_msg': 'Permission denied.'
                    })
                ...
                for repo_id in valid_repo_id_list:
                    ...
                    try:
                        ...
                        send_perm_audit_msg('add-repo-perm', username, to_username,
                                            repo_id, '/', permission)
                        send_perm_audit_signal(request, 'add-repo-perm', repo_id, '/', permission, to_username, 'user_email')
                    except Exception as e:
                        logger.error(e)
                        result['failed'].append({
                            'repo_id': repo_id,
                ...
                for repo_id in valid_repo_id_list:
                    ...
                    try:
                        ...
                        send_perm_audit_msg('add-repo-perm', username, to_group_id,
                                            repo_id, '/', permission)
                        send_perm_audit_signal(request, 'add-repo-perm', repo_id, '/', permission, to_group_id, 'group')    
...
class ReposBatchCreateDirView(APIView):
    ...
    def post(self, request):
        ...
        for path in path_list:
            ...
            try:
                # TODO
                # move syncwerk_api.mkdir_with_parents() to CE version
