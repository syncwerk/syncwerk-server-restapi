...
from restapi.utils.slugify import slugify
from restapi.utils import gen_file_get_url, get_file_type_and_ext, \
...
from restapi.wiki.models import WikiPageMissing, WikiDoesNotExist, GroupWiki, PersonalWiki
...
def get_wiki_dirent(repo_id, page_name):
    ...
    cmmt = synserv.get_commits(repo.id, 0, 1)[0]
    if cmmt is None:
        raise WikiPageMissing
    dirs = syncwerk_api.list_dir_by_commit_and_path(cmmt.repo_id, cmmt.id, "/")
    if dirs:
        for e in dirs:
...
def get_inner_file_url(repo, obj_id, file_name):
    repo_id = repo.id
    access_token = syncwerk_api.get_fileserver_access_token(repo_id, obj_id,
                                                           'view', '')
...
def get_personal_wiki_repo(username):
    try:
        ...
        raise WikiDoesNotExist
    repo = synserv.get_repo(wiki.repo_id)
    if not repo:
        raise WikiDoesNotExist
    return repo
...
def get_group_wiki_repo(group, username):
    ...
    repos = synserv.get_group_repos(group.id, username)
    for repo in repos:
...
def get_wiki_pages(repo):
    ...
    """
    dir_id = synserv.syncwserv_threaded_rpc.get_dir_id_by_path(repo.id, '/')
    if not dir_id:
        return {}
    dirs = syncwerk_api.list_dir_by_dir_id(repo.id, dir_id)
    pages = {}
    for e in dirs:
        if stat.S_ISDIR(e.mode):
            continue            # skip directories
...
def convert_wiki_link(content, url_prefix, repo_id, username):
    ...
    def repl(matchobj):
        ...
        if fileext == '':
            ...
            token = syncwerk_api.get_fileserver_access_token(repo_id, obj_id,
                                                            'view', username)
...
def get_wiki_page_object(wiki_object, page_name):
    ...
    try:
        dirent = syncwerk_api.get_dirent_by_path(repo_id, filepath)
        if dirent:
    ...
    try:
        repo = syncwerk_api.get_repo(wiki_object.repo_id)
    except RpcsyncwerkError as e:
        logger.error(e)
...
def get_wiki_dirs_by_path(repo_id, path, all_dirs):
    dirs = syncwerk_api.list_dir_by_path(repo_id, path)
