...
class ShareSearchUser(APIView):
    ...
    def _can_use_global_address_book(self, request):
        ...
        if not ENABLE_GLOBAL_ADDRESSBOOK:
            return False
    ...
    @swagger_auto_schema(
        ...
        operation_summary='Search users to share',
        operation_description='''Search user from contacts/all users to share''',
        tags=['shares'],
        manual_parameters=[
            openapi.Parameter(
                name='repo_id',
            ...
            openapi.Parameter(
                ...
                in_="query",
                type='string',
                description='0 or 1. Include the current login user in the result or not',
            ),
        ],
        responses={
    ...
    def get(self, request, repo_id, format=None):
        ...
        shared_users = self.list_user_shared_items(request, repo_id, path)
        ...
        unshared_result = [val for val in email_result if val not in shared_users]

        formated_result = format_searched_user_result(
                request, unshared_result[:10], size)
...
def search_user_from_ccnet(q):
    ...
    # all_db_users = synserv.get_emailusers('DB', -1, -1)
    # for db_user in all_db_users:
    #     print db_user
    #     if q in db_user.email:
