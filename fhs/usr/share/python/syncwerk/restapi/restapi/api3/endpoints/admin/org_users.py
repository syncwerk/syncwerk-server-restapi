...
try:
    from restapi.settings import ORG_MEMBER_QUOTA_ENABLED
except ImportError:
...
def get_org_user_info(org_id, email):
    ...
    org_user_quota = syncwerk_api.get_org_user_quota(org_id, email)
    user_info['quota_total'] = org_user_quota / get_file_size_unit('MB')

    org_user_quota_usage = syncwerk_api.get_org_user_quota_usage(org_id, email)
...
class AdminOrgUser(APIView):
    ...
    def put(self, request, org_id, email):
        ...
        if user_quota_mb:
            ...
            org_quota = syncwerk_api.get_org_quota(org_id)
            ...
            syncwerk_api.set_org_user_quota(org_id, email, user_quota)
