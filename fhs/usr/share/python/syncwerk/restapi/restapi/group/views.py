...
from restapi.group.utils import validate_group_name, BadGroupNameError, \
    ConflictGroupNameError, is_group_member
from restapi.wiki.models import WikiDoesNotExist, WikiPageMissing, GroupWiki
from restapi.wiki.utils import (clean_page_name, page_name_to_file_name,
                               get_wiki_pages, get_group_wiki_repo,
                               get_group_wiki_page)
from restapi.settings import SITE_ROOT
from restapi.utils import render_error, send_html_email, is_org_context, \
    get_site_name
from restapi.views import is_registered_user, check_folder_permission
from restapi.views.modules import get_enabled_mods_by_group, \
    get_available_mods_by_group
from restapi.share.models import ExtraGroupsSharePermission
from restapi.constants import HASH_URLS

from restapi.forms import SharedRepoCreateForm
...
def create_group(group_name, username):
    return synserv.ccnet_threaded_rpc.create_group(group_name, username)

def create_org_group(org_id, group_name, username):
    return synserv.ccnet_threaded_rpc.create_org_group(org_id, group_name,
                                                       username)

def get_all_groups(start, limit):
    return synserv.ccnet_threaded_rpc.get_all_groups(start, limit)

def org_user_exists(org_id, username):
    return synserv.ccnet_threaded_rpc.org_user_exists(org_id, username)

########## helper functions
def is_group_staff(group, user):
    if user.is_anonymous():
        return False
    return synserv.check_group_staff(group.id, user.username)
...
def create_group_repo(request, group_id):
    ...
    if is_org_context(request):
        ...
        try:
            syncwerk_api.set_group_repo(repo_id, group.id, username, permission)
        except RpcsyncwerkError, e:
            logger.error(e)
            return json_error(_(u'Failed to create: internal error.'))
        else:
...
def group_wiki(request, group, page_name="home"):
    ...
    try:
        ...
        try:
            dirent = syncwerk_api.get_dirent_by_path(repo.id, path)
            if dirent:
        ...
        if is_registered_user(username):
            repo_perm = syncwerk_api.check_permission_by_path(repo.id, '/', username)
        else:
...
def group_wiki_create(request, group):
    ...
    repo_id = syncwerk_api.create_repo(repo_name, repo_desc, user, passwd)
    if not repo_id:
        return json_error(_(u'Failed to create'), 500)

    try:
        syncwerk_api.set_group_repo(repo_id, group.id, user, permission)
    except RpcsyncwerkError as e:
        remove_repo(repo_id)
...
def group_wiki_use_lib(request, group):
    ...
    page_name = "home.md"
    if not synserv.get_file_id_by_path(repo_id, "/" + page_name):
        if not synserv.post_empty_file(repo_id, "/", page_name, username):
