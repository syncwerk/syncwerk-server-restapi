...
def check_file_lock(repo_id, file_path, username):
    ...
    According to returned value of syncwerk_api.check_file_lock:
    0: not locked
    1: locked by other
    2: locked by me
    -1: error
    ...
    return_value = syncwerk_api.check_file_lock(repo_id,
    ...
    if return_value == 0:
        ...
        raise RpcsyncwerkError('check file lock error')

def if_locked_by_online_office(repo_id, path):

    locked_by_online_office = False
    if is_pro_version():
        lock_info = syncwerk_api.get_lock_info(repo_id, path)
