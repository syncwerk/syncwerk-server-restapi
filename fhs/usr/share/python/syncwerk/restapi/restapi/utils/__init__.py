...
from restapi.auth import REDIRECT_FIELD_NAME
from restapi.api2.models import Token, TokenV2
import restapi.settings
from restapi.settings import SITE_NAME, MEDIA_URL, LOGO_PATH, \
        MEDIA_ROOT, CUSTOM_LOGO_PATH

from restapi.signals import send_email_signal

try:
    from restapi.settings import EVENTS_CONFIG_FILE
except ImportError:
...
try:
    ...
    CLOUD_MODE = False
try:
    from restapi.settings import ENABLE_INNER_FILESERVER
except ImportError:
    ENABLE_INNER_FILESERVER = True
try:
    from restapi.settings import CHECK_SHARE_LINK_TRAFFIC
except ImportError:
    CHECK_SHARE_LINK_TRAFFIC = False

def is_cluster_mode():
    cfg = ConfigParser.ConfigParser()
    if 'SYNCWERK_CENTRAL_CONF_DIR' in os.environ:
        confdir = os.environ['SYNCWERK_CENTRAL_CONF_DIR']
    else:
        confdir = os.environ['SYNCWERK_CONF_DIR']
    conf = os.path.join(confdir, 'server.conf')
    cfg.read(conf)
...
def is_allow_edit_history_keep_days():
    cfg = ConfigParser.ConfigParser()
    if 'SYNCWERK_CENTRAL_CONF_DIR' in os.environ:
        confdir = os.environ['SYNCWERK_CENTRAL_CONF_DIR']
    else:
    ...
    conf = os.path.join(confdir, 'server.conf')
    cfg.read(conf)
    if cfg.has_option('history', 'allow_edit_keep_day'):
        enabled = cfg.getboolean('history', 'allow_edit_keep_day')
    else:
        enabled = False
    return enabled

ALLOW_EDIT_HISTORY_KEEP_DAY = is_allow_edit_history_keep_days()

try:
    from restapi.settings import OFFICE_CONVERTER_ROOT
except ImportError:
    OFFICE_CONVERTER_ROOT = ''
try:
    from restapi.settings import OFFICE_CONVERTER_NODE
except ImportError:
    OFFICE_CONVERTER_NODE = False

from restapi.utils.file_types import *
from restapi.utils.htmldiff import HtmlDiff # used in views/files.py
...
def get_fileserver_root():
    """ Construct syncwerk fileserver address and port.
...
def get_inner_fileserver_root():
    """Construct inner syncwerk fileserver address and port.

    Inner fileserver root allows Restapi access fileserver through local
    ...
    return restapi.settings.INNER_FILE_SERVER_ROOT
...
def get_repo_last_modify(repo):
    ...
    if repo.head_cmmt_id is not None:
        ...
        last_cmmt = syncwerk_api.get_commit_list(repo.id, 0, 1)[0]
...
def is_valid_dirent_name(name):
    ...
    """
    # `repo_id` parameter is not used in syncwerk api
    return syncwerk_api.is_valid_filename('fake_repo_id', name)
...
def check_filename_with_rename(repo_id, parent_dir, obj_name):
    cmmts = syncwerk_api.get_commit_list(repo_id, 0, 1)
    latest_commit = cmmts[0] if cmmts else None
    if not latest_commit:
        return ''
    # TODO: what if parrent_dir does not exist?
    dirents = syncwerk_api.list_dir_by_commit_and_path(repo_id,
...
def get_user_repos(username, org_id=None):
    ...
    """
    if org_id is None:
        owned_repos = syncwerk_api.get_owned_repo_list(username)
        shared_repos = syncwerk_api.get_share_in_repo_list(username, -1, -1)
        groups_repos = syncwerk_api.get_group_repos_by_user(username)
        if CLOUD_MODE:
            public_repos = []
        else:
            public_repos = synserv.list_inner_pub_repos(username)
        ...
        shared_repos = syncwerk_api.get_org_share_in_repo_list(org_id,
                username, -1, -1)
        groups_repos = syncwerk_api.get_org_group_repos_by_user(username,
                org_id)
        public_repos = synserv.syncwserv_threaded_rpc.list_org_inner_pub_repos(org_id)
...
def get_file_revision_id_size(repo_id, commit_id, path):
    """Given a commit and a file path in that commit, return the syncwerk id
    ...
    syncwdir = syncwerk_api.list_dir_by_commit_and_path(repo_id, commit_id, dirname)
    for dirent in syncwdir:
...
def get_commit_before_new_merge(commit):
    ...
    while(new_merge_with_no_conflict(commit)):
        p1 = synserv.get_commit(commit.repo_id, commit.version, commit.parent_id)
        p2 = synserv.get_commit(commit.repo_id, commit.version, commit.second_parent_id)
...
def get_syncwerk_server_ccnet_addr_port():
    """get ccnet server host and port"""
    return synserv.CCNET_SERVER_ADDR, synserv.CCNET_SERVER_PORT
...
def is_org_context(request):
    """An organization context is a virtual private Syncwerk instance on cloud
    service.
...
if EVENTS_CONFIG_FILE:
    ...
    # EVENTS_ENABLED = True
    # SyncwEventsSession = syncwevents.init_db_session_class(EVENTS_CONFIG_FILE)

    # @contextlib.contextmanager
    # def _get_syncwevents_session():
    #     try:
    #         session = SyncwEventsSession()
    #         yield session
    #     finally:
    #        session.close()
    ...
    #     '''Read events from syncwevents database, and remove events that are
    #     no longer valid
    ...
    #     while True:
    #         if org_id > 0:
    #             events = syncwevents.get_org_user_events(ev_session, org_id,
    #                                                     username, next_start,
    #                                                     limit)
    #         else:
    #             events = syncwevents.get_user_events(ev_session, username,
    #                                                 next_start, limit)
    #         if not events:
    #             break
    ...
    #                 repo = syncwerk_api.get_repo(ev.repo_id)
    #                 if not repo:
    #                     # delete the update event for repo which has been deleted
    #                     syncwevents.delete_event(ev_session, ev.uuid)
    #                     continue
    #                 if repo.encrypted:
    #                     repo.password_set = syncwerk_api.is_password_set(
    #                         repo.id, username)
    #                 ev.repo = repo
    #                 ev.commit = synserv.get_commit(repo.id, repo.version, ev.commit_id)
    ...
    #     """
    #     """
    #     with _get_syncwevents_session() as session:
    #         res = syncwevents.get_user_activity_stats_by_day(session, start, end, offset)
    #     return res
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         events = syncwevents.get_event_log_by_time(session, log_type, tstart, tend)
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         events = syncwevents.get_file_audit_events_by_path(session,
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         events = syncwevents.get_file_audit_events(session, email, org_id, repo_id, start, limit)
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         res = syncwevents.get_file_ops_stats_by_day(session, start, end, offset)
    #     return res
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         res = syncwevents.get_total_storage_stats_by_day(session, start, end, offset)
    #     return res

    # def get_system_traffic_by_day(start, end, offset, op_type='all'):
    #     with _get_syncwevents_session() as session:
    #         res = syncwevents.get_system_traffic_by_day(session, start, end, offset, op_type)
    #     return res

    # def get_org_traffic_by_day(org_id, start, end, offset, op_type='all'):
    #     with _get_syncwevents_session() as session:
    #         res = syncwevents.get_org_traffic_by_day(session, org_id, start, end, offset, op_type)
    #     return res
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         events = syncwevents.get_file_update_events(session, email, org_id, repo_id, start, limit)
    #     return events if events else None
    ...
    #     """
    #     with _get_syncwevents_session() as session:
    #         events = syncwevents.get_perm_audit_events(session, email, org_id, repo_id, start, limit)

    #     return events if events else None

    # def get_virus_record(repo_id=None, start=-1, limit=-1):
    #     with _get_syncwevents_session() as session:
    #         r = syncwevents.get_virus_record(session, repo_id, start, limit)
    #     return r if r else []

    # def handle_virus_record(vid):
    #     with _get_syncwevents_session() as session:
    #         return True if syncwevents.handle_virus_record(session, vid) == 0 else False

    # def get_virus_record_by_id(vid):
    #     with _get_syncwevents_session() as session:
    #         return syncwevents.get_virus_record_by_id(session, vid)
else:
    EVENTS_ENABLED = False
    def get_user_events():
        pass
...
def get_service_url():
    """Get service url from synserv.
    """
    return config.SERVICE_URL

def get_server_id():
    """Get server id from synserv.
    """
    return getattr(synserv, 'SERVER_ID', '-')
...
def get_site_scheme_and_netloc():
    ...
    For example:
    >>> get_site_scheme_and_netloc("https://example.com:8000/syncwerk/")
...
#         enabled = syncwevents.is_audit_enabled(parsed_events_conf)
...
#         enabled = syncwevents.is_office_converter_enabled(parsed_events_conf)
...
#         return syncwevents.get_office_converter_dir(parsed_events_conf, 'html')

#     def get_office_converter_pdf_dir():
#         return syncwevents.get_office_converter_dir(parsed_events_conf, 'pdf')

#     def get_office_converter_limit():
#         return syncwevents.get_office_converter_limit(parsed_events_conf)
...
if HAS_OFFICE_CONVERTER:
    ...
    from syncwevents.office_converter import OfficeConverterRpcClient
    ...
    def _get_office_converter_rpc():
        ...
        if office_converter_rpc is None:
            pool = ccnet.ClientPool(
                synserv.CCNET_CONF_PATH,
                central_config_dir=synserv.SYNCWERK_CENTRAL_CONF_DIR
            )
    ...
    def _office_convert_token_header(file_id):
        return {
            'X-Syncwerk-Office-Preview-Token': office_convert_cluster_token(file_id),
        }
    ...
    def cluster_delegate(delegate_func):
        ...
        def decorated(func):
            def real_func(*args, **kwargs):
                ...
                if CLUSTER_MODE and not OFFICE_CONVERTER_NODE and not cluster_internal:
    ...
    def delegate_add_office_convert_task(file_id, doctype, raw_path):
        url = urljoin(OFFICE_CONVERTER_ROOT, '/office-convert/internal/add-task/')
    ...
    def delegate_query_office_convert_status(file_id, doctype):
        url = urljoin(OFFICE_CONVERTER_ROOT, '/office-convert/internal/status/')
    ...
    def delegate_get_office_converted_page(request, repo_id, commit_id, path, static_filename, file_id):
        url = urljoin(OFFICE_CONVERTER_ROOT,
...
#         enabled = False
#         if hasattr(syncwevents, 'is_search_enabled'):
#             enabled = syncwevents.is_search_enabled(parsed_events_conf)
...
# init Syncwevents API
if EVENTS_CONFIG_FILE:
    # from syncwevents import syncwevents_api
    # syncwevents_api.init(EVENTS_CONFIG_FILE)
    syncwevents_api = None
else:
    syncwevents_api = None       # TODO
...
def user_traffic_over_limit(username):
    ...
    from restapi_extra.plan.settings import PLAN
    ...
    try:
        stat = syncwevents_api.get_user_traffic_by_month(username, datetime.now())
...
def is_pro_version():
    return True
    if restapi.settings.DEBUG:
        if hasattr(restapi.settings, 'IS_PRO_VERSION') \
            and restapi.settings.IS_PRO_VERSION:
            return True
    if EVENTS_CONFIG_FILE:
        return True
    else:
...
def clear_token(username):
    ...
    syncwerk_api.delete_repo_tokens_by_email(username)
...
def send_perm_audit_msg(etype, from_user, to, repo_id, path, perm):
    ...
    try:
        synserv.send_message('restapi.audit', msg_utf8)
    except Exception as e:
...
def get_folder_permission_recursively(username, repo_id, path):
    ...
    if not syncwerk_api.get_dir_id_by_path(repo_id, path):
       # get current folder's parent directory
        ...
        return syncwerk_api.check_permission_by_path(
