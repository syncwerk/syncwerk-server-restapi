...
from synserv import get_file_id_by_path, get_repo, get_file_size, \
    syncwerk_api

from restapi.utils import gen_inner_file_get_url, get_file_type_and_ext
from restapi.utils.file_types import VIDEO
from restapi.settings import THUMBNAIL_IMAGE_SIZE_LIMIT, \
...
def generate_thumbnail(request, repo_id, size, path):
    ...
    token = syncwerk_api.get_fileserver_access_token(repo_id,
            file_id, 'view', username, use_onetime=False)
    if not token:
        return (False, 500)

    inner_path = gen_inner_file_get_url(token, os.path.basename(path))
    print 'THUMBNAIL CAN RUN THORUGH HERE'
    print inner_path
    ...
    try:
        ...
        logger.error(e)
        print 'THUMBNAIL CAN RUN THORUGH ERROR'
        print e
        return (False, 500)
...
def create_video_thumbnails(repo, file_id, path, size, thumbnail_file, file_size):
    ...
    t1 = timeit.default_timer()
    token = syncwerk_api.get_fileserver_access_token(repo.id,
