...
from restapi.base.decorators import user_mods_check
from restapi.wiki.models import PersonalWiki, WikiDoesNotExist, WikiPageMissing
from restapi.wiki.utils import (get_personal_wiki_page,
                               get_personal_wiki_repo, get_wiki_pages,
                               clean_page_name, page_name_to_file_name)
from restapi.wiki.forms import WikiCreateForm, WikiNewPageForm
...
def personal_wiki(request, page_name="home"):
    ...
    if request.cloud_mode and request.user.org is not None:
        ...
        joined_groups = synserv.get_personal_groups_by_user(username)
    ...
    try:
        ...
        if not synserv.post_empty_file(repo.id, "/", filename, username):
...
def personal_wiki_create(request):
    ...
    repo_id = syncwerk_api.create_repo(repo_name, repo_desc, username, passwd)
    if not repo_id:
...
def personal_wiki_page_new(request, page_name="home"):
    ...
    if request.method == 'POST':
        ...
        if synserv.get_file_id_by_path(repo.id, filepath):
            return render_error(request, _('Page "%s" already exists.') % filename)

        if not synserv.post_empty_file(repo.id, "/", filename, request.user.username):
            return render_error(request, _('Failed to create wiki page. Please retry later.'))
...
def personal_wiki_page_delete(request, page_name):
    ...
    username = request.user.username
    if synserv.del_file(repo.id, '/', file_name, username):
