...
from restapi.base.decorators import sys_staff_required, require_POST
...
from restapi.auth.decorators import login_required, login_required_ajax
from restapi.constants import GUEST_USER, DEFAULT_USER, DEFAULT_ADMIN, \
...
from restapi.two_factor.models import default_device
...
from restapi.utils.licenseparse import parse_license, user_number_over_limit
...
from restapi.utils.repo import get_related_users_by_repo, get_repo_owner
from restapi.views import get_system_default_repo_id
from restapi.forms import SetUserQuotaForm, AddUserForm, BatchAddUserForm, \
...
from restapi.settings import INIT_PASSWD, SITE_ROOT, \
...
def sysadmin(request):
    ...
    try:
        expire_days = syncwerk_api.get_server_config_int('library_trash', 'expire_days')
    ...
    return render(request, 'sysadmin/sysadmin_backbone.html', {
            ...
            'enable_encrypted_library': config.ENABLE_ENCRYPTED_FOLDER,
...
def sys_statistic_traffic(request):
    ...
    if req_type == 'user':
        traffic_info_list = syncwevents_api.get_all_users_traffic_by_month(
            month_dt, start, limit, order_by)
    else:
        traffic_info_list = syncwevents_api.get_all_orgs_traffic_by_month(
            month_dt, start, limit, order_by)
...
def sys_user_admin(request):
    ...
    try:
        ...
        current_page = 1
        per_page = 25
    users_plus_one = synserv.get_emailusers('DB', per_page * (current_page - 1),
                                            per_page + 1)
...
def sys_user_admin_ldap_imported(request):
    ...
    try:
        ...
        current_page = 1
        per_page = 25
    users_plus_one = synserv.get_emailusers('LDAPImport',
...
def sys_user_admin_ldap(request):
    ...
    try:
        ...
        current_page = 1
        per_page = 25
    users_plus_one = synserv.get_emailusers('LDAP',
                                            per_page * (current_page - 1),
                                            per_page + 1)
    if len(users_plus_one) == per_page + 1:
        page_next = True
...
def user_set_quota(request, email):
    ...
    if f.is_valid():
        ...
        try:
            if not org:
                syncwerk_api.set_user_quota(email, space_quota)
            else:
                org_id = org[0].org_id
                org_quota_mb = syncwserv_threaded_rpc.get_org_quota(org_id) / get_file_size_unit('MB')
                if space_quota_mb > org_quota_mb:
                    result['error'] = _(u'Failed to set quota: maximum quota is %d MB' % \
                                            org_quota_mb)
                    return HttpResponse(json.dumps(result), status=400, content_type=content_type)
                else:
                    syncwerk_api.set_org_user_quota(org_id, email, space_quota)
        except:
...
def sys_org_set_quota(request, org_id):
    ...
    try:
        syncwserv_threaded_rpc.set_org_quota(org_id, quota)
    except RpcsyncwerkError as e:
        logger.error(e)
        result['error'] = _(u'Failed to set quota: internal server error')
        return HttpResponse(json.dumps(result), status=500, content_type=content_type)
...
def sys_org_admin(request):
    ...
    try:
        from restapi_extra.plan.models import OrgPlan
    ...
    if enable_org_plan and request.GET.get('filter', '') == 'paid':
        ...
        for e in ops:
            ...
            o.quota_usage = syncwserv_threaded_rpc.get_org_quota_usage(o.org_id)
            o.total_quota = syncwserv_threaded_rpc.get_org_quota(o.org_id)
    ...
    for org in orgs:
        org.quota_usage = syncwserv_threaded_rpc.get_org_quota_usage(org.org_id)
        org.total_quota = syncwserv_threaded_rpc.get_org_quota(org.org_id)

        from restapi_extra.organizations.settings import ORG_TRIAL_DAYS
        if ORG_TRIAL_DAYS > 0:
...
def sys_org_remove(request, org_id):
    ...
    # remove org repos
    syncwserv_threaded_rpc.remove_org_repo_by_org_id(org_id)
...
def sys_get_org_base_info(org_id):
    ...
    # quota
    total_quota = syncwserv_threaded_rpc.get_org_quota(org_id)
    quota_usage = syncwserv_threaded_rpc.get_org_quota_usage(org_id)
...
def sys_org_info_user(request, org_id):
    ...
    for user in users:
        if user.email == request.user.email:
            user.is_self = True
        try:
            user.self_usage = syncwerk_api.get_org_user_quota_usage(org_id, user.email)
            user.quota = syncwerk_api.get_org_user_quota(org_id, user.email)
        except RpcsyncwerkError as e:
            logger.error(e)
            user.self_usage = -1
            user.quota = -1
...
def sys_org_info_library(request, org_id):
    ...
    # library
    org_repos = syncwserv_threaded_rpc.get_org_repo_list(org_id, -1, -1)

    for repo in org_repos:
        try:
            repo.owner = syncwserv_threaded_rpc.get_org_repo_owner(repo.id)
        except:
            repo.owner = None
...
def sys_repo_delete(request, repo_id):
    ...
    try:
        org_id = syncwerk_api.get_org_id_by_repo_id(repo_id)
...
def sys_virus_scan_records(request):
    ...
    for r in records_all[:per_page]:
        try:
            repo = syncwerk_api.get_repo(r.repo_id)
        except RpcsyncwerkError as e:
            logger.error(e)
            continue
        ...
        r.repo = repo
        r.repo.owner = syncwerk_api.get_repo_owner(r.repo.repo_id)
        records.append(r)
...
def sys_delete_virus_scan_records(request, vid):
    ...
    try:
        syncwerk_api.del_file(r.repo_id, parent_dir, dirent_name,
    ...
    if request.method == 'POST':
        ...
        if password:
            ...
            if user:
                ...
                from restapi.auth.utils import clear_login_failed_attempts
                clear_login_failed_attempts(request, username)

                return HttpResponseRedirect(next)
        password_error = True

        from restapi.auth.utils import get_login_failed_attempts, incr_login_failed_attempts
        failed_attempt = get_login_failed_attempts(username=username, ip=ip)
        if failed_attempt >= config.LOGIN_ATTEMPT_LIMIT:
            # logout user
            from restapi.auth import logout
            logout(request)
...
def sys_settings(request):
    """List and change restapi settings in admin panel.
    """
    ...
    DIGIT_WEB_SETTINGS = [
        ...
        'ENABLE_ENCRYPTED_FOLDER', 'USER_PASSWORD_MIN_LENGTH',
...
def sys_check_license(request):
    """Check syncwerk license expiration.
    """
    if not is_pro_version():
        raise Http404
...
def sys_inst_admin(request):
    ...
    offset = per_page * (current_page - 1)
    insts = Tenant.objects.all()[offset:offset + per_page + 1]
...
def sys_inst_add_user(request, inst_id):
    ...
    try:
        ...
        return HttpResponse(json.dumps({'error': "Tenant does not exist"}),
                status=400)
    ...
    for email in email_list:
        try:
            ...
            messages.error(request, u'Failed to add %s to the tenant: user does not exist.' % email)
            continue
        ...
        if profile.tenant:
            messages.error(request, _(u"Failed to add %s to the tenant: user already belongs to an tenant") % email)
            continue
        else:
            profile.tenant = inst.name
        profile.save()
        messages.success(request, _(u'Successfully added %s to the tenant.') % email)
...
def sys_inst_info_user(request, inst_id):
    """List tenant members including admins.
    """
    try:
        inst = Tenant.objects.get(pk=inst_id)
    except Tenant.DoesNotExist:
...
def sys_inst_toggle_admin(request, inst_id, email):
    ...
    if u.is_staff:
        messages.error(
            request, 'Can not assign tenantal administration roles to global administrators')
        return HttpResponseRedirect(next)

    res = TenantAdmin.objects.filter(tenant=inst, user=email)
    if len(res) == 0:
        TenantAdmin.objects.create(tenant=inst, user=email)
    elif len(res) == 1:
        res[0].delete()
