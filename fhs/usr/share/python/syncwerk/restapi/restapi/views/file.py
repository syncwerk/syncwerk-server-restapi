...
from restapi.tags.models import FileUUIDMap
from restapi.wopi.utils import get_wopi_dict
from restapi.onlyoffice.utils import get_onlyoffice_dict
from restapi.auth.decorators import login_required
from restapi.base.decorators import repo_passwd_set_required
from restapi.base.accounts import ANONYMOUS_EMAIL
from restapi.share.models import FileShare, check_share_link_common
from restapi.share.decorators import share_link_audit, share_link_login_required
...
from restapi.utils.file_types import (IMAGE, PDF, SVG,
        DOCUMENT, SPREADSHEET, AUDIO, MARKDOWN, TEXT, VIDEO)
from restapi.utils.star import is_file_starred
from restapi.utils.http import json_response, \
...
from restapi.utils.repo import is_repo_owner
...
from restapi.settings import FILE_ENCODING_LIST, FILE_PREVIEW_MAX_SIZE, \
    FILE_ENCODING_TRY_LIST, MEDIA_URL, SYNCWERK_COLLAB_SERVER, ENABLE_WATERMARK
...
try:
    from restapi.onlyoffice.settings import ONLYOFFICE_FILE_EXTENSION
except ImportError:
    ONLYOFFICE_FILE_EXTENSION = ()

try:
    from restapi.onlyoffice.settings import ONLYOFFICE_EDIT_FILE_EXTENSION
except ImportError:
...
def can_preview_file(file_name, file_size, repo):
    """Check whether Syncwerk supports view file.
    Returns (True, None) if Yes, otherwise (False, error_msg).
    """

    filetype, fileext = get_file_type_and_ext(file_name)

    # Syncwerk defines 9 kinds of filetype:
    ...
    if filetype in (TEXT, MARKDOWN, IMAGE) or fileext in get_conf_text_ext():
        ...
        # OOS > OnlyOffice > Syncwerk integrated
        ...
        # NOT depends on Syncwerk settings
...
def can_edit_file(file_name, file_size, repo):
    """Check whether Syncwerk supports edit file.
    Returns (True, None) if Yes, otherwise (False, error_msg).
    """
...
def view_lib_file(request, repo_id, path):
    ...
    file_size = syncwerk_api.get_file_size(repo.store_id, repo.version, file_id)
    ...
    if filetype in (TEXT, MARKDOWN) or fileext in get_conf_text_ext():
        ...
        if filetype == MARKDOWN:
            ...
            return_dict['syncwerk_collab_server'] = SYNCWERK_COLLAB_SERVER
        ...
        img_list = []
        dirs = syncwerk_api.list_dir_by_path(repo_id, parent_dir)
        for dirent in dirs:
        ...
        if ENABLE_OFFICE_WEB_APP:
            ...
            if wopi_dict:
                if is_pro_version() and action_name == 'edit':
                    try:
                        if not is_locked:
                            syncwerk_api.lock_file(repo_id, path, ONLINE_OFFICE_LOCK_OWNER, 0)
...
def view_shared_file(request, fileshare):
    ...
    if not syncwerk_api.check_permission_by_path(repo_id, '/', shared_by):
    ...
    # get raw file
    access_token = syncwerk_api.get_fileserver_access_token(repo_id,
...
def send_file_access_msg(request, repo, path, access_from):
    ...
    try:
        send_message('restapi.audit', msg_utf8)
    except Exception as e:
