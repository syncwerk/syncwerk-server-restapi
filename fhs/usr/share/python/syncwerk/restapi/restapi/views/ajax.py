...
import synserv
from synserv import syncwerk_api, is_passwd_set, ccnet_api, \
...
from restapi.forms import RepoRenameDirentForm
...
from restapi.views import get_unencry_rw_repos_by_user, \
    get_diff, check_folder_permission
from restapi.group.utils import is_group_member, is_group_admin_or_owner, \
    get_group_member_info
import restapi.settings as settings
from restapi.settings import ENABLE_THUMBNAIL, THUMBNAIL_ROOT, \
    THUMBNAIL_DEFAULT_SIZE, SHOW_TRAFFIC, MEDIA_URL, ENABLE_VIDEO_THUMBNAIL
from restapi.utils import check_filename_with_rename, EMPTY_SHA1, \
    gen_block_get_url, \
...
from restapi.utils.file_op import check_file_lock, ONLINE_OFFICE_LOCK_OWNER
from restapi.utils.repo import get_locked_files_by_dir, get_repo_owner
...
def get_unenc_group_repos(request, group_id):
    ...
    if is_org_context(request):
        ...
        repos = syncwerk_api.get_org_group_repos(org_id, group_id_int)
        for repo in repos:
            if not repo.encrypted:
                repo_list.append({"name": repo.repo_name, "id": repo.repo_id})
    else:
        repos = syncwerk_api.get_repos_by_group(group_id_int)
        for repo in repos:
            if not repo.encrypted:
...
def convert_repo_path_when_can_not_view_folder(request, repo_id, path):
    ...
    converted_repo_path = syncwerk_api.convert_repo_path(repo_id, path, username)
    ...
    if repo_owner == username:
        ...
        try:
            if is_org_context(request):
                ...
                if syncwerk_api.org_repo_has_been_shared(repo_id, including_groups=True) or is_inner_org_pub_repo:
                    result["has_been_shared_out"] = True
            else:
                if syncwerk_api.repo_has_been_shared(repo_id, including_groups=True) or \
...
def rename_dirent(request, repo_id):
    ...
    if syncwerk_api.get_dir_id_by_path(repo.id, full_path) is not None:
    ...
    if syncwerk_api.get_file_id_by_path(repo.id, full_path) is not None:
    ...
    # rename file/dir
    try:
        syncwerk_api.rename_file(repo_id, parent_dir, oldname, newname, username)
    except RpcsyncwerkError, e:
        result['error'] = str(e)
...
def delete_dirent(request, repo_id):
    ...
    # delete file/dir
    try:
        syncwerk_api.del_file(repo_id, parent_dir, dirent_name, username)
        return HttpResponse(json.dumps({'success': True}),
                            content_type=content_type)
    except RpcsyncwerkError, e:
        logger.error(e)
...
def mv_dirents(request, src_repo_id, src_path, dst_repo_id, dst_path,
    ...
    if get_repo_owner(request, src_repo_id) != get_repo_owner(request, dst_repo_id):
        ...
        total_size = 0
        repo = syncwerk_api.get_repo(src_repo_id)
        for obj_name in obj_file_names + obj_dir_names:

            current_size = 0
            current_path = posixpath.join(src_path, obj_name)

            current_file_id = syncwerk_api.get_file_id_by_path(src_repo_id,
                    current_path)
            if current_file_id:
...
def space_and_traffic(request):
    ...
    if not org:
        ...
        space_usage = syncwerk_api.get_org_user_quota_usage(org_id, username)
...
def get_file_upload_url_ul(request, token):
    ...
    if repo.encrypted or \
            syncwerk_api.check_permission_by_path(repo_id, '/', shared_by) != 'rw':
...
def _create_repo_common(request, repo_name, repo_desc, encryption,
    ...
    try:
        if not encryption:
            ...
            if is_org_context(request):
                org_id = request.user.org.org_id
                repo_id = syncwerk_api.create_org_enc_repo(
...
def ajax_repo_dir_recycle_more(request, repo_id):
    ...
    try:
        ...
        deleted_entries = syncwerk_api.get_deleted(repo_id, 0, path, scan_stat)
    except RpcsyncwerkError as e:
        logger.error(e)
        result['error'] = 'Internal server error'
        return HttpResponse(json.dumps(result), status=500,
