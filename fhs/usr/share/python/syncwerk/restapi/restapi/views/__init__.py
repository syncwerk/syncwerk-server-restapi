...
import synserv
from synserv import get_repo, get_commits, \
    syncwserv_threaded_rpc, syncwserv_rpc, is_repo_owner, \
    get_file_size, MAX_DOWNLOAD_DIR_SIZE, \
    syncwerk_api, ccnet_api
from pyrpcsyncwerk import RpcsyncwerkError

from restapi.avatar.util import get_avatar_file_storage
from restapi.auth.decorators import login_required
from restapi.auth import login as auth_login
from restapi.auth import get_backends
from restapi.base.accounts import User
from restapi.base.decorators import user_mods_check, require_POST
...
from restapi.revision_tag.models import RevisionTags
from restapi.utils import render_permission_error, render_error, \
...
from restapi.utils.repo import get_library_storages
...
from restapi.views.modules import MOD_PERSONAL_WIKI, enable_mod_for_user, \
    disable_mod_for_user
import restapi.settings as settings
from restapi.settings import AVATAR_FILE_STORAGE, \
...
from restapi.wopi.settings import ENABLE_OFFICE_WEB_APP
from restapi.onlyoffice.settings import ENABLE_ONLYOFFICE
...
def check_folder_permission(request, repo_id, path):
    ...
    return syncwerk_api.check_permission_by_path(repo_id, path, username)
...
def get_repo_dirents(request, repo, commit, path, offset=-1, limit=-1):
    ...
    if commit.root_id == EMPTY_SHA1:
        ...
        try:
            dirs = syncwerk_api.list_dir_by_commit_and_path(commit.repo_id,
                                                           commit.id, path,
                                                           offset, limit)
            if not dirs:
                return ([], [], False)
...
def create_default_library(request):
    ...
    if is_org_context(request):
        ...
        default_repo = syncwerk_api.create_org_repo(name=_("My Library"),
                                                   desc=_("My Library"),
                                                   username=username,
                                                   passwd=None,
                                                   org_id=org_id)
    else:
        default_repo = syncwerk_api.create_repo(name=_("My Library"),
                                               desc=_("My Library"),
                                               username=username,
                                               passwd=None)
...
def libraries(request):
    ...
    if request.cloud_mode and request.user.org is not None:
        org_id = request.user.org.org_id
        joined_groups = synserv.get_org_groups_by_user(org_id, username)
    else:
...
def repo_set_access_property(request, repo_id):
    ...
    syncwserv_threaded_rpc.repo_set_access_property(repo_id, ap)
    reverse_url = HASH_URLS["VIEW_COMMON_LIB_DIR"] % {'repo_id': repo_id, 'path': ''}
...
def repo_download_dir(request, repo_id):
    ...
    if not syncwerk_api.get_dir_id_by_path(repo.id, path):
    ...
    if allow_download:
        ...
        dir_id = syncwerk_api.get_dir_id_by_commit_and_path(repo.id,
...
def convert_cmmt_desc_link(request):
    ...
    diff_result = syncwserv_threaded_rpc.get_diff(repo_id, '', cmmt_id)
    if not diff_result:
        raise Http404
