...
from restapi.settings import SYNCWERK_VERSION, SITE_TITLE, SITE_NAME, \
...
try:
    from restapi.settings import SEACLOUD_MODE
except ImportError:
    SEACLOUD_MODE = False

from restapi.utils import HAS_FILE_SEARCH, EVENTS_ENABLED, is_pro_version

try:
    from restapi.settings import ENABLE_PUBFILE
except ImportError:
    ENABLE_PUBFILE = False
try:
    from restapi.settings import ENABLE_SYSADMIN_EXTRA
except ImportError:
...
def base(request):
    """
    Add restapi base configure to the context.
    ...
    result = {
        'syncwerk_version': SYNCWERK_VERSION,
