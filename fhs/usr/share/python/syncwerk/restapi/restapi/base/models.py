...
from restapi.utils import calc_file_path_hash, within_time_range
    ...
    def get_starred_files_by_username(self, username):
        ...
        for sfile in starred_files:
            ...
            if repo_cache.has_key(sfile.repo_id):
                ...
                try:
                    repo = syncwerk_api.get_repo(sfile.repo_id)
            ...
            if sfile.path != "/":
                try:
                    file_id = syncwerk_api.get_file_id_by_path(sfile.repo_id,
                                                              sfile.path)
                    # size = syncwerk_api.get_file_size(file_id)
                except RpcsyncwerkError:
                    continue
                if not file_id:
                    sfile.delete()
        ...
        for sfile in ret:
            ...
            try:
                ...
                dirent = syncwerk_api.get_dirent_by_path(sfile.repo.store_id,
                                                        real_path)
                if dirent:
