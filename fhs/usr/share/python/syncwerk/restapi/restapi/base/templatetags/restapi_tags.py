...
from restapi.profile.settings import NICKNAME_CACHE_TIMEOUT, NICKNAME_CACHE_PREFIX, \
    EMAIL_ID_CACHE_TIMEOUT, EMAIL_ID_CACHE_PREFIX, CONTACT_CACHE_TIMEOUT, \
    CONTACT_CACHE_PREFIX
from restapi.cconvert import CConvert
from restapi.po import TRANSLATION_MAP
from restapi.shortcuts import get_first_object_or_none
from restapi.utils import normalize_cache_key, CMMT_DESC_PATT
from restapi.utils.html import avoid_wrapping
...
def translate_commit_desc(value):
    ...
    if value.startswith('Reverted repo'):
        ...
        # only used inside of syncwerk system.
    ...
    if value.startswith('Reverted library'):
        ...
        return _('Auto merge by syncwerk system')
...
def translate_commit_desc_escape(value):
    ...
    if value.startswith('Reverted library'):
        ...
        return_value = escape(_('Auto merge by syncwerk system'))
    elif value.startswith('Created library'):
...
@register.filter(name='translate_restapi_time')
def translate_restapi_time(value, autoescape=None):
    ...
    translated_time = translate_restapi_time_str(val)
    if autoescape:
...
def translate_restapi_time_str(val):
...
@register.filter(name='restapi_urlize')
def restapi_urlize(value, autoescape=None):
    """Converts URLs in plain text into clickable links."""
    from restapi.base.utils import urlize
    return mark_safe(urlize(value, nofollow=True, autoescape=autoescape))
restapi_urlize.is_safe=True
restapi_urlize.needs_autoescape = True
...
def restapi_filesizeformat(bytes):
    """
