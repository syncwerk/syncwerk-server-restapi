...
from restapi.role_permissions.utils import get_enabled_role_permissions_by_role, \
        get_enabled_admin_role_permissions_by_role
from restapi.utils import is_user_password_strong, get_site_name, \
...
class AdminPermissions(object):
    ...
    def can_generate_share_link(self):
        return get_enabled_admin_role_permissions_by_role(self.user.admin_role)['can_generate_share_link']

    def can_generate_upload_link(self):
        return get_enabled_admin_role_permissions_by_role(self.user.admin_role)['can_generate_upload_link']
class User(object):
    is_staff = False
    is_active = False
    is_superuser = False
    ...
    def save(self):
        ...
        if emailuser:
            logger.info('IT JUST UPDATE PASSWORD FOR CURRENT USER')
            ...
            backed_up_is_active = self.is_active
            ccnet_threaded_rpc.update_emailuser(source,
                                                emailuser.id,
                                                self.password,
                                                int(self.is_staff),
                                                0)
            self.is_active = backed_up_is_active
            ...
            logger.info('IT ADDING A NEWUSER')
    ...
    def delete(self):
        ...
        if orgs:
            for org in orgs:
                owned_repos += syncwerk_api.get_org_owned_repo_list(org.org_id,
                                                                   username)
        else:
            owned_repos += syncwerk_api.get_owned_repo_list(username)

        for r in owned_repos:
            syncwerk_api.remove_repo(r.id)
        ...
        if orgs:
            for org in orgs:
                org_id = org.org_id
                shared_in_repos = syncwerk_api.get_org_share_in_repo_list(org_id,
                        username, -1, -1)

                for r in shared_in_repos:
                    syncwerk_api.org_remove_share(org_id,
                            r.repo_id, r.user, username)
        else:
            shared_in_repos = syncwerk_api.get_share_in_repo_list(username, -1, -1)
            for r in shared_in_repos:
                syncwerk_api.remove_share(r.repo_id, r.user, username)
    ...
    def remove_repo_passwds(self):
        ...
        """
        from restapi.utils import get_user_repos
...
class AuthBackend(object):
    ...
    def get_user_with_import(self, username):
        emailuser = synserv.get_emailuser_with_import(username)
        if not emailuser:
            raise User.DoesNotExist, 'User matching query does not exits.'
