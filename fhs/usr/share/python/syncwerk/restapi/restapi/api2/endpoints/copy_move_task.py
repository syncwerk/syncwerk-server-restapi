...
class CopyMoveTaskView(APIView):
    ...
    def post(self, request):
        ...
        file_id = None
        if dirent_type == 'file':
            file_id = syncwerk_api.get_file_id_by_path(src_repo_id, src_dirent_path)
            if not file_id:
        ...
        dir_id = None
        if dirent_type == 'dir':
            dir_id = syncwerk_api.get_dir_id_by_path(src_repo_id, src_dirent_path)
            if not dir_id:
        ...
        if operation == 'copy' or \
            ...
            current_size = 0
            if file_id:
                current_size = syncwerk_api.get_file_size(src_repo.store_id,
