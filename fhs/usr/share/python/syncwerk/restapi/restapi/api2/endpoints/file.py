...
from restapi.utils.file_op import check_file_lock, if_locked_by_online_office
from restapi.views.file import can_preview_file, can_edit_file
...
class FileView(APIView):
    ...
    def put(self, request, repo_id, format=None):
        ...
        if operation == 'lock':
            ...
            try:
                syncwerk_api.lock_file(repo_id, path, username, expire)
            except RpcsyncwerkError, e:
                logger.error(e)
                error_msg = 'Internal Server Error'
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
        ...
        if operation == 'unlock':
            ...
            if locked_by_me or locked_by_online_office:
                # unlock file
                try:
                    syncwerk_api.unlock_file(repo_id, path)
                except RpcsyncwerkError, e:
                    logger.error(e)
                    error_msg = 'Internal Server Error'
                    return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
