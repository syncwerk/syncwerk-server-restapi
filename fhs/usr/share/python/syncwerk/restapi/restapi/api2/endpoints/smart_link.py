...
from restapi.utils import get_service_url, normalize_dir_path, \
        normalize_file_path
from restapi.utils.repo import is_valid_repo_id_format
from restapi.views import check_folder_permission

from restapi.api2.utils import to_python_boolean
...
class SmartLink(APIView):
    ...
    def get(self, request):
        ...
        if is_dir:
            if not syncwerk_api.get_dir_id_by_path(repo_id, normalize_dir_path(path)):
                error_msg = 'Folder %s not found.' % path
                return api_error(status.HTTP_404_NOT_FOUND, error_msg)
        else:
            if not syncwerk_api.get_file_id_by_path(repo_id, normalize_file_path(path)):
