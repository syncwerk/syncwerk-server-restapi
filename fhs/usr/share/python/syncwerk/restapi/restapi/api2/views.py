...
from .utils import get_diff_details, to_python_boolean, \
    ...
    get_groups, api_group_check, get_timestamp, json_response, is_syncwerk_pro
...
from restapi.base.models import UserStarredFiles, DeviceToken, RepoSecretKey
from restapi.share.models import ExtraSharePermission, ExtraGroupsSharePermission
from restapi.share.utils import is_repo_admin, check_group_share_in_permission
...
from restapi.group.utils import BadGroupNameError, ConflictGroupNameError, \
...
from restapi.signals import (repo_created, repo_deleted)
from restapi.auth.signals import user_logged_in_success_event, user_logged_in_failed_event
from restapi.share.models import FileShare, OrgFileShare, UploadLinkShare
...
from restapi.utils.file_revisions import get_file_revisions_after_renamed
from restapi.utils.devices import do_unlink_device
from restapi.utils.repo import get_repo_owner, get_library_storages, \
...
from restapi.utils.file_types import DOCUMENT
from restapi.utils.file_size import get_file_size_unit
from restapi.utils.file_op import check_file_lock
...
from restapi.views import is_registered_user, check_folder_permission, \
    create_default_library, list_inner_pub_repos
from restapi.views.file import get_file_view_path_and_perm, send_file_access_msg
if HAS_FILE_SEARCH:
    from restapi_extra.search.utils import search_files, get_search_repos_map, SEARCH_FILEEXT
from restapi.utils import HAS_OFFICE_CONVERTER
if HAS_OFFICE_CONVERTER:
    from restapi.utils import query_office_convert_status, prepare_converted_html
import restapi.settings as settings
from restapi.settings import THUMBNAIL_EXTENSION, THUMBNAIL_ROOT, \
...
try:
    ...
    MULTI_TENANCY = False
try:
    from restapi.settings import ORG_MEMBER_QUOTA_DEFAULT
except ImportError:
...
class ObtainAuthToken(APIView):
    ...
    def post(self, request):
        ...
        if serializer.two_factor_auth_failed:
            ...
            # for the 2fa token.
            headers = {
                'X-Syncwerk-OTP': 'required',
            }
        # Send login failed signal
        try:
            user_logged_in_failed_event.send(sender=None, request=request, device_name=request.data.get('device_name', None))
        except Exception as e:
            logger.error(e)
...
class Search(APIView):
    ...
    def get(self, request, format=None):
        ...
        if search_path:
            ...
            dir_id = syncwerk_api.get_dir_id_by_path(search_repo, search_path)
            if not dir_id:
...
class Repos(APIView):
    ...
    def get(self, request, format=None):
        ...
        if filter_by['group']:
            if is_org_context(request):
                org_id = request.user.org.org_id
                group_repos = syncwerk_api.get_org_group_repos_by_user(email,
                        org_id)
            else:
                group_repos = syncwerk_api.get_group_repos_by_user(email)
        ...
        try:
            send_message('restapi.stats', 'user-login\t%s\t%s\t%s' % (email, timestamp, org_id))
        except Exception as e:
    ...
    def _create_repo(self, request, repo_name, repo_desc, username, org_id):
        ...
        if org_id > 0:
            repo_id = syncwerk_api.create_org_repo(repo_name,
            ...
            if is_pro_version() and ENABLE_STORAGE_CLASSES:
                ...
                if STORAGE_CLASS_MAPPING_POLICY in ('USER_SELECT',
                    ...
                    repo_id = syncwerk_api.create_repo(repo_name,
    ...
    def _create_enc_repo(self, request, repo_id, repo_name, repo_desc, username, org_id):
        if not config.ENABLE_ENCRYPTED_FOLDER:
...
class PubRepos(APIView):
    ...
    def post(self, request, format=None):
        ...
        if is_org_context(request):
            ...
            repo = syncwerk_api.get_repo(repo_id)
            syncwerk_api.set_org_inner_pub_repo(org_id, repo.id, permission)
        else:
            if is_pro_version() and ENABLE_STORAGE_CLASSES:
...
class FileBlockDownloadLinkView(APIView):
    ...
    def get(self, request, repo_id, file_id, block_id, format=None):
        ...
        token = syncwerk_api.get_fileserver_access_token(
...
class UploadBlksLinkView(APIView):
    ...
    def get_blklist_missing(self, repo_id, blks):
        ...
        blklist = blks.split(',')
        try:
            return json.loads(syncwerk_api.check_repo_blocks_missing(
...
def get_dir_file_recursively(username, repo_id, path, all_dirs):
    ...
    dirs = syncwerk_api.list_dir_with_perm(repo_id, path,
    ...
    def post(self, request, repo_id, format=None):
        ...
        if get_repo_owner(request, repo_id) != get_repo_owner(request, dst_repo):
            ...
            for obj_name in obj_names:
                ...
                current_file_id = syncwerk_api.get_file_id_by_path(repo_id,
                        current_path)
                if current_file_id:
                    current_size = syncwerk_api.get_file_size(repo.store_id,
                            repo.version, current_file_id)

                current_dir_id = syncwerk_api.get_dir_id_by_path(repo_id,
                        current_path)
                if current_dir_id:
                    current_size = syncwerk_api.get_dir_size(repo.store_id,
                            repo.version, current_dir_id)

                total_size += current_size

            # check if above quota for dst repo
            if syncwerk_api.check_quota(dst_repo, total_size) < 0:
                return api_error(HTTP_443_ABOVE_QUOTA, _(u"Out of quota."))
...
class FileView(APIView):
    ...
    def post(self, request, repo_id, format=None):
        ...
        if operation.lower() == 'rename':
            ...
            if not syncwerk_api.is_valid_filename('fake_repo_id', newname):
            ...
            try:
                syncwerk_api.rename_file(repo_id, parent_dir, oldname, newname,
                                        username)
            except RpcsyncwerkError,e:
                return api_error(HTTP_520_OPERATION_FAILED,
            ...
            try:
                syncwerk_api.move_file(src_repo_id, src_dir,
            ...
            new_filename = check_filename_with_rename(dst_repo_id, dst_dir, filename)
            try:
                syncwerk_api.copy_file(src_repo_id, src_dir,
                                      filename, dst_repo_id,
                                      dst_dir, new_filename,
            ...
            if not syncwerk_api.is_valid_filename('fake_repo_id', new_file_name):
                error_msg = 'File name invalid.'
                return api_error(status.HTTP_400_BAD_REQUEST, error_msg)
            ...
            try:
                syncwerk_api.post_empty_file(repo_id, parent_dir,
    ...
    def put(self, request, repo_id, format=None):
        ...
        if operation.lower() == 'lock':
            ...
            try:
                syncwerk_api.lock_file(repo_id, path.lstrip('/'), username, expire)
                return Response('success', status=status.HTTP_200_OK)
            except RpcsyncwerkError, e:
                logger.error(e)
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, 'Internal error')
        ...
        if operation.lower() == 'unlock':
            ...
            # unlock file
            try:
                syncwerk_api.unlock_file(repo_id, path.lstrip('/'))
                return Response('success', status=status.HTTP_200_OK)
            except RpcsyncwerkError, e:
                logger.error(e)
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, 'Internal error')
...
class FileDetailView(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        try:
            if commit_id:
                obj_id = syncwerk_api.get_file_id_by_commit_and_path(repo_id,
                        commit_id, path)
            else:
        ...
        try:
            # get real path for sub repo
            dirent = syncwerk_api.get_dirent_by_path(real_repo_id, real_path)
        except Exception as e:
            logger.error(e)
            dirent = None
...
class FileHistory(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        return HttpResponse(json.dumps({"commits": commits},
            cls=RpcsyncwerkObjEncoder), status=200, content_type=json_content_type)
...
class DirView(APIView):
    ...
    def post(self, request, repo_id, format=None):
        ...
        if operation.lower() == 'mkdir':
            ...
            if not syncwerk_api.is_valid_filename('fake_repo_id', new_dir_name):
            ...
            if not create_parents:
                ...
                try:
                    syncwerk_api.mkdir_with_parents(repo_id, '/',
                                                   path[1:], username)
                except RpcsyncwerkError as e:
                    logger.error(e)
                    return api_error(HTTP_520_OPERATION_FAILED,
...
class DirRevert(APIView):
    ...
    def put(self, request, repo_id):
        ...
        if not syncwerk_api.get_dir_id_by_commit_and_path(repo_id, commit_id, path):
            error_msg = 'folder %s not found.' % path
            return api_error(status.HTTP_404_NOT_FOUND, error_msg)
...
class SharedRepo(APIView):
    ...
    def delete(self, request, repo_id, format=None):
        ...
        if share_type == 'personal':
            ...
            syncwerk_api.unset_group_repo(repo_id, int(group_id), username)
        elif share_type == 'public':
            if is_org_context(request):
                org_id = request.user.org.org_id
                synserv.syncwserv_threaded_rpc.unset_org_inner_pub_repo(org_id, repo_id)
            else:
    ...
    def put(self, request, repo_id, format=None):
        ...
        if share_type == 'personal':
            ...
            for u in usernames:
                ...
                try:
                    syncwerk_api.share_repo(repo_id, username, u, permission)
        ...
        if share_type == 'group':
            ...
            try:
                syncwerk_api.set_group_repo(repo_id,
                        group_id, username, permission)
            except RpcsyncwerkError, e:
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR,
                                 "Rpcsyncwerk Error: " + e.msg)
            try:
                send_perm_audit_msg('add-repo-perm',
        ...
        if share_type == 'public':
            try:
                if is_org_context(request):
                    org_id = request.user.org.org_id
                    syncwerk_api.set_org_inner_pub_repo(org_id, repo_id, permission)
                else:
                    if not request.user.permissions.can_add_public_repo():
                        error_msg = 'Permission denied.'
                        return api_error(status.HTTP_403_FORBIDDEN, error_msg)
...
class Groups(APIView):
    ...
    def delete(self, request, group_id, format=None):
        ...
        # permission check
        username = request.user.username
        if not synserv.check_group_staff(group_id, username):
    ...
    def post(self, request, group_id, format=None):
        ...
        # permission check
        username = request.user.username
        if not synserv.check_group_staff(group.id, username):
...
class GroupRepo(APIView):
    ...
    def delete(self, request, group, repo_id, format=None):
        ...
        is_org = synserv.is_org_group(group_id)
        repo = syncwerk_api.get_group_shared_repo_by_path(repo_id, None, group_id, is_org)
...
class RepoTokensView(APIView):
    ...
    def get(self, request, format=None):
        ...
        for repo_id in repos_id:
            ...
            tokens[repo_id] = syncwerk_api.generate_repo_token(repo_id, request.user.username)
...
class OrganizationView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            # set member limit
            from restapi_extra.organizations.models import OrgMemberQuota
            OrgMemberQuota.objects.set_quota(new_org.org_id, member_limit)

            # set quota
            quota = quota_mb * get_file_size_unit('MB')
            syncwserv_threaded_rpc.set_org_quota(new_org.org_id, quota)
...
class RepoGroupFolderPerm(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        results = []
        path = request.GET.get('folder_path', None)
        group_folder_perms = syncwerk_api.list_folder_group_perm_by_repo(repo_id)
    ...
    def post(self, request, repo_id, format=None):
        ...
        for group_id in group_ids:
            ...
            permission = syncwerk_api.get_folder_group_perm(repo_id, path, group_id)
            if permission:
                result['failed'].append({
                    'group_id': group_id,
                    'error_msg': 'Permission already exists.'
            ...
            try:
                syncwerk_api.add_folder_group_perm(repo_id, path, perm, group_id)
            ...
            new_perm = syncwerk_api.get_folder_group_perm(repo_id, path, group_id)
    ...
    def put(self, request, repo_id, format=None):
        ...
        # modify permission
        try:
            syncwerk_api.set_folder_group_perm(repo_id, path, perm, group_id)
    ...
    def delete(self, request, repo_id, format=None):
        ...
        try:
            syncwerk_api.rm_folder_group_perm(repo_id, path, group_id)
            send_perm_audit_msg('delete-repo-perm', username, group_id,
