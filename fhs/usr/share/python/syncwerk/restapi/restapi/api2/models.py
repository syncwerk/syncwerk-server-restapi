...
class TokenV2Manager(models.Manager):
    ...
    def get_devices(self, platform, start, end):
        ...
        if platform == 'desktop':
            devices = devices.filter(platform__in=DESKTOP_PLATFORMS).order_by(
                '-last_accessed')[start: end]
        elif platform == 'mobile':
            devices = devices.filter(platform__in=MOBILE_PLATFORMS).order_by(
                '-last_accessed')[start: end]
        else:
    ...
    def get_user_devices(self, username):
        ...
        devices = super(TokenV2Manager, self).filter(
            user=username).filter(wiped_at=None)
        platform_priorities = {
            'windows': 0,
            'linux': 0,
            'mac': 0,
        ...
        def sort_devices(d1, d2):
            ...
            '''
            ret = cmp(platform_priorities[d1.platform],
                      platform_priorities[d2.platform])
            if ret != 0:
                return ret
    ...
    def delete_device_token(self, username, platform, device_id):
        super(TokenV2Manager, self).filter(user=username,
                                           platform=platform, device_id=device_id).delete()
