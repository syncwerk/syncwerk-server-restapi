...
from restapi.signals import repo_deleted
...
from restapi.group.utils import is_group_member, group_id_to_name
from restapi.utils.repo import get_related_users_by_repo
...
class AdminLibrary(APIView):
    ...
    def put(self, request, repo_id, format=None):
        ...
        if new_repo_name:
            try:
                res = syncwerk_api.edit_repo(repo_id, new_repo_name, '', None)
            except Exception as e:
                logger.error(e)
                error_msg = 'Internal Server Error'
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
        ...
        if new_owner:
            ...
            if MULTI_TENANCY:
                try:
                    if syncwerk_api.get_org_id_by_repo_id(repo_id) > 0:
