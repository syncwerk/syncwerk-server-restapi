...
from restapi.base.templatetags.restapi_tags import email2nickname, \
        email2contact_email
from restapi.utils import gen_file_upload_url, gen_shared_upload_link
...
class AdminUploadLinkUpload(APIView):
    ...
    def get(self, request, token):
        ...
        obj_id = syncwerk_api.get_dir_id_by_path(repo_id, path)
        if not obj_id:
            error_msg = 'Folder not found.'
            return api_error(status.HTTP_404_NOT_FOUND, error_msg)

        upload_token = syncwerk_api.get_fileserver_access_token(repo_id,
