...
from restapi.api2.endpoints.utils import generate_links_header_for_paginator
...
from restapi.profile.settings import CONTACT_CACHE_TIMEOUT, CONTACT_CACHE_PREFIX
...
def update_user_info(request, user):
    ...
    if quota_total_mb:
        ...
        if is_org_context(request):
            ...
            syncwerk_api.set_org_user_quota(org_id, email, quota_total)
        else:
            syncwerk_api.set_user_quota(email, quota_total)
...
def get_user_info(email):
    ...
    info['quota_total'] = syncwerk_api.get_user_quota(email)
    info['quota_usage'] = syncwerk_api.get_user_self_usage(email)
...
class AdminUsers(APIView):
    ...
    def post(self, request):
        ...
        if quota_total_mb:
            ...
            if is_org_context(request):
                org_id = request.user.org.org_id
                org_quota_mb = syncwerk_api.get_org_quota(org_id) / \
