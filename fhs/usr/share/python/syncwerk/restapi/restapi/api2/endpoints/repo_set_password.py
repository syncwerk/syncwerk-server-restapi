...
from restapi.utils.repo import is_repo_owner, add_encrypted_repo_secret_key_to_database
from restapi.base.models import RepoSecretKey
from restapi.views import check_folder_permission

from restapi.settings import ENABLE_RESET_ENCRYPTED_REPO_PASSWORD
...
class RepoSetPassword(APIView):
    ...
    def put(self, request, repo_id):
        ...
        if operation == 'reset-password':
            ...
            try:
                syncwerk_api.reset_repo_passwd(repo_id, username, secret_key, new_password)
            except Exception as e:
                logger.error(e)
                error_msg = 'Internal Server Error'
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
