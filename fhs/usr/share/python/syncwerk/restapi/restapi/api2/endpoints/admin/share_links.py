...
from restapi.utils import gen_file_get_url, gen_dir_zip_download_url, \
...
def get_share_link_info(fileshare):
    ...
    if fileshare.s_type == 'f':
        ...
        data['size'] = syncwerk_api.get_file_size(repo.store_id,
                repo.version, obj_id)
...
class AdminShareLinkDirents(APIView):
    ...
    def get(self, request, token):
        ...
        obj_id = syncwerk_api.get_dir_id_by_path(repo_id, obj_path)
        if not obj_id:
        ...
        real_obj_id = syncwerk_api.get_dir_id_by_path(repo_id, real_path)
        if not real_obj_id:
            error_msg = 'Folder %s not found.' % req_path
            return api_error(status.HTTP_404_NOT_FOUND, error_msg)

        try:
            current_commit = syncwerk_api.get_commit_list(repo_id, 0, 1)[0]
            dirent_list = syncwerk_api.list_dir_by_commit_and_path(repo_id,
...
class AdminShareLinkDownload(APIView):
    ...
    def get(self, request, token):
        ...
        if sharelink.s_type == 'f':
            ...
            obj_id = syncwerk_api.get_file_id_by_path(repo_id, obj_path)
            if not obj_id:
            ...
            try:
                ...
                download_token = syncwerk_api.get_fileserver_access_token(repo_id,
            ...
            if download_type == 'file':
                ...
                real_obj_id = syncwerk_api.get_file_id_by_path(repo_id, real_path)
                if not real_obj_id:
                    error_msg = 'File not found.'
                    return api_error(status.HTTP_404_NOT_FOUND, error_msg)
                ...
                try:
                    zip_token = syncwerk_api.get_fileserver_access_token(repo_id,
