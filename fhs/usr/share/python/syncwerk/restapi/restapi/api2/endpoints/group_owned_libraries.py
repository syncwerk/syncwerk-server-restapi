...
from restapi.api2.endpoints.utils import (
...
from restapi.utils.rpc import SyncwerkAPI
from restapi.share.signals import share_repo_to_user_successful, share_repo_to_group_successful
from restapi.share.utils import share_dir_to_user, share_dir_to_group, update_user_dir_permission, \
...
class GroupOwnedLibraries(APIView):
    ...
    def post(self, request, group_id, org_id):
        ...
        if password and not config.ENABLE_ENCRYPTED_FOLDER:
        ...
        if is_pro_version() and ENABLE_STORAGE_CLASSES:
            ...
            if STORAGE_CLASS_MAPPING_POLICY in ('USER_SELECT',
                ...
                repo_id = SyncwerkAPI.add_group_owned_repo(
...
class GroupOwnedLibrary(APIView):
    ...
    def put(self, request, group_id, repo_id):
        ...
        try:
            repo_owner = get_repo_owner(request, repo_id)
            # desc is ''
            syncwerk_api.edit_repo(repo_id, new_repo_name, '', repo_owner)
        except Exception as e:
            logger.error(e)
    ...
    def delete(self, request, group_id, repo_id, org_id):
        ...
        try:
            SyncwerkAPI.delete_group_owned_repo(group_id, repo_id, org_id)
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
...
class GroupOwnedLibraryUserShare(APIView):
    ...
    def list_user_shared_items(self, request, repo_id, path, org_id):
        repo_owner = get_repo_owner(request, repo_id)
        share_items = SyncwerkAPI.get_shared_users_by_repo_path(
    ...
    def delete(self, request, repo_id, org_id, format=None):
        ...
        path = request.data.get('path', '/')
        SyncwerkAPI.delete_shared_user_by_repo_path(
...
class GroupOwnedLibraryGroupShare(APIView):
    ...
    def list_group_shared_items(self, request, repo_id, path, org_id):
        repo_owner = get_repo_owner(request, repo_id)
        share_items = SyncwerkAPI.get_shared_groups_by_repo_path(
        ...
        for item in share_items:
            ...
            if not group:
                SyncwerkAPI.delete_shared_group_by_repo_path(
...
class GroupOwnedLibraryUserShareInLibrary(APIView):
    ...
    def delete(self, request, repo_id, org_id, format=None):
        ...
        try:
            if org_id:
                is_org = True
                syncwerk_api.org_remove_share(org_id, repo_id, repo_owner, username)
            else:
                is_org = False
                syncwerk_api.remove_share(repo_id, repo_owner, username)
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
