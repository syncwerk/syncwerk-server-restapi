...
def get_dirent_name_list(username, repo_id, parent_path):
    ...
    path_id = syncwerk_api.get_dir_id_by_path(repo_id, parent_path)
    dirs = syncwerk_api.list_dir_with_perm(repo_id, parent_path,
...
class MoveFolderMergeView(APIView):
    ...
    def post(self, request):
        ...
        dir_id = syncwerk_api.get_dir_id_by_path(src_repo_id, src_folder_path)
        if not dir_id:
        ...
        if not syncwerk_api.get_dir_id_by_path(dst_repo_id, dst_parent_dir):
        ...
        if get_repo_owner(request, src_repo_id) != get_repo_owner(request, dst_repo_id):
            ...
            if syncwerk_api.check_quota(dst_repo_id, current_size) < 0:
        ...
        syncwerk_api.del_file(src_repo_id, src_parent_dir, src_folder_name, username)
