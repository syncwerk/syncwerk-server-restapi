        ...
        $('#confirm-yes').data('url', $(this).data('url')).on('click', function() {
            if (popup.post) { // use form post
                $('<form>', {
                    ...
                    "html": '<input name="csrfmiddlewaretoken" value="' + getCookie(SYNCWERK_GLOBAL.csrfCookieName) + '" type="hidden">'
...
function prepareCSRFToken(xhr, settings) {
    if (!(/^http:.*/.test(settings.url) || /^https:.*/.test(settings.url))) {
        ...
        xhr.setRequestHeader("X-CSRFToken", getCookie(SYNCWERK_GLOBAL.csrfCookieName));
    }
}
